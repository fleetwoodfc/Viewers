...
- **Transaction UID**: A per-claim identifier generated by the performer SCU
  when claiming one specific workitem (one UID per claim, not per AE).
  Required for all subsequent N-SET and N-ACTION requests (Complete, Cancel)
